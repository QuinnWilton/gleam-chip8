...
pub fn disassemble(instruction: Instruction) -> String {
  case instruction {
    ...
    SkipNextIfNotEqualRegisters(vx, vy) ->
      string.join(
        [
          "SNE",
          registers.data_register_to_string(vx),
    ...
    WaitForKeyPress(vx) ->
      string.join(["LD", registers.data_register_to_string(vx), "K"], with: " ")

    SetDelayTimerToRegisterValue(vx) ->
      string.join(
        ["LD", "DT", registers.data_register_to_string(vx)],
        with: " ",
      )

    SetSoundTimerToRegisterValue(vx) ->
      string.join(
        ["LD", "ST", registers.data_register_to_string(vx)],
        with: " ",
      )

    AddToAddressRegister(vx) ->
      string.join(
        ["ADD", "I", registers.data_register_to_string(vx)],
        with: " ",
      )

    SetAddressRegisterToSpriteLocation(vx) ->
      string.join(["LD", "F", registers.data_register_to_string(vx)], with: " ")

    StoreBcdOfRegister(vx) ->
      string.join(["LD", "B", registers.data_register_to_string(vx)], with: " ")

    StoreRegistersAtAddressRegister(vx) ->
      string.join(
        ["LD", "[I]", registers.data_register_to_string(vx)],
        with: " ",
      )

    ReadRegistersFromAddressRegister(vx) ->
      string.join(
